...
if [[ "${AI_GATEWAY_ENV:-development}" == "production" ]]; then
  for v in ODOO_COMMIT_SHA ODOO_LLM_COMMIT_SHA VLLM_VERSION PYTHON_RUNTIME_VERSION NODE_RUNTIME_VERSION PGVECTOR_PACKAGE MODEL_REVISION_QWEN MODEL_REVISION_GLIMMER MODEL_REVISION_VISION MODEL_REVISION_EMBEDDING AI_GATEWAY_ALLOWED_ORIGIN AI_GATEWAY_REDIS_URL; do
