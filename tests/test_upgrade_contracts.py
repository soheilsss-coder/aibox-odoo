...
class SourceContracts(unittest.TestCase):
    ...
    def test_buzz_is_a_group_channel_agent_surface(self):
        api = (ADDONS / "ai_collaboration/controllers/api.py").read_text()
        link = (ADDONS / "ai_collaboration/models/channel_link.py").read_text()
        page = (ROOT / "frontend/src/pages/BuzzPage.jsx").read_text()
        app = (ROOT / "frontend/src/App.jsx").read_text()
        for marker in (
            "/api/collaboration/channels/<int:channel_id>/messages",
            "_reply_to_channel_message",
            "_group_context",
            "trigger_text",
            "agent_name",
        ):
            self.assertIn(marker, api + link)
        self.assertIn("postBuzzMessage", page)
        self.assertIn("/buzz", app)
