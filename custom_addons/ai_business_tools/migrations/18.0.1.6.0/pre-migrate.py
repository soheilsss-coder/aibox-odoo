...
def migrate(cr, version):
    ...
    """)
    cr.execute("""
        ALTER TABLE ai_gateway_access_grant
        ADD COLUMN IF NOT EXISTS company_id integer
    """)
    cr.execute("""
        UPDATE ai_gateway_access_grant
           SET company_id = COALESCE(
               (SELECT company_id FROM res_users WHERE res_users.id = ai_gateway_access_grant.to_user_id),
    ...
    cr.execute("""
        ...
        BEGIN
            ...
            END IF;
            IF EXISTS (SELECT 1 FROM ai_gateway_access_grant WHERE company_id IS NULL) THEN
                RAISE EXCEPTION 'Cannot backfill ai_gateway_access_grant.company_id';
            END IF;
            ...
            ALTER TABLE ai_gateway_access_grant ALTER COLUMN company_id SET NOT NULL;
        END $$;
    """)
    cr.execute("""
    ...
    """)
    cr.execute("""
        CREATE INDEX IF NOT EXISTS ai_gateway_access_grant_company_id_idx
        ON ai_gateway_access_grant (company_id)
    """)
