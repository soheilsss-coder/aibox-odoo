...
class LlmApiConfig:
    ...
    ``chat_key`` / ``embedding_key`` hold real credentials, so they are
    ``repr=False`` and deliberately absent from ``describe()`` - that method is
    what gets returned to the browser and written into seed output, and a
    redacted view that leaks on a stray ``%r`` would defeat the point.
    """
...
def load_config(env: Optional[Dict[str, str]] = None) -> LlmApiConfig:
    ...
    return LlmApiConfig(
        ...
        notes=notes,
        chat_key=chat_key,
        embedding_key=(emb_key or chat_key or "").strip(),
    )
