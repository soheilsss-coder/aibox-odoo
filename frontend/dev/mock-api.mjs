...
let branding = { brand_name: "Nova Enterprise", brand_domain: "nova.example.com", company_name: "Nova Demo Inc.", has_logo: true };
...
function cannedReply(message) {
  return (
    ...
    "This answer comes from the development mock server, not the real model. " +
...
async function handle(req, res) {
  ...
  if (path === "/api/login" && method === "POST") {
    ...
    if (!body.login || !body.password) return json(res, 400, { error: "Email and password are both required." });
  ...
  if (!sid) return json(res, 401, { error: "Your session is not valid. Please sign in." });
  ...
  if (path === "/api/workspace" && method === "GET") return json(res, 200, { department: { id: 1, name: "Sales" }, user: USER.name });
  ...
  if (path === "/api/tasks") {
    ...
    if (method === "POST") { const b = await readBody(req); const t = { id: nextId++, name: b.name || "Untitled task", state: "open" }; tasks.unshift(t); return json(res, 200, { task: t }); }
  }
  ...
  if (path === "/api/hr/leaves") {
    ...
    if (method === "POST") {
      ...
      if (!b.date_from || !b.date_to) return json(res, 400, { error: "A date range is required." });
      const l = { id: nextId++, date_from: b.date_from, date_to: b.date_to, leave_type: "Annual leave", reason: b.reason || "", status: "pending_approval" };
      leaves.unshift(l);
  ...
  if (path === "/api/documents" && method === "POST") {
    ...
    const doc = { id: nextId++, name: b.name || "Untitled document", description: b.description || "", access_level: b.access_level || "personal", department: dept ? dept.name : undefined, group: b.group_id ? "Contracts group" : undefined, owner: USER.name, file_name: b.file_name || "", create_date: new Date().toISOString().slice(0, 19).replace("T", " "), is_mine: true };
    documents.unshift(doc);
    return json(res, 200, { document: doc });
  }
  m = path.match(/^\/api\/documents\/(\d+)$/);
  if (m && method === "GET") { const d = documents.find((x) => x.id === Number(m[1])); return d ? json(res, 200, { document: d }) : json(res, 404, { error: "Document not found." }); }
  if (m && method === "DELETE") { documents = documents.filter((x) => x.id !== Number(m[1])); return json(res, 200, { ok: true }); }
  if (path === "/api/documents/options" && method === "GET") return json(res, 200, { departments: departments.map((d) => ({ id: d.id, name: d.name })), groups: [{ id: 5, name: "Contracts group" }], own_department_id: 1 });
  if (path === "/api/documents/search" && method === "POST") {
    ...
    const results = (hits.length ? hits : documents.slice(0, 2)).map((d, i) => ({ document_id: d.id, document_name: d.name, similarity: (0.92 - i * 0.07).toFixed(2), excerpt: `${d.description || d.name} — a sample excerpt from the document for semantic-search UI testing.` }));
    return json(res, 200, { results });
  }

  // --- files / artifacts (Chat page attachments) ---
  if (path === "/api/files/analyze" && method === "POST") { const b = await readBody(req); return json(res, 200, { analysis: `Simulated analysis of “${b.filename || "file"}”: a text document with general information; key takeaway: this is demo data from the mock server.` }); }
  ...
  if (path === "/api/admin/access-grants" && method === "POST") {
    ...
    const g = { id: nextId++, to_user: u ? u.name : "User", role: roles[Number(b.group_id) % roles.length]?.name || "Base role", expires_on: b.expires_on || "—", state: "active" };
    grants.unshift(g);
  ...
  json(res, 404, { error: `mock: unknown route ${method} ${path}` });
}
