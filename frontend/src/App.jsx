...
function AgentsPage(){const [items,setItems]=useState([]);const [error,setError]=useState("");useEffect(()=>{api.getAgents().then(x=>setItems(x.agents||[])).catch(e=>setError(e.message||"خطا در بارگذاری دستیارها"))},[]);return <><PageHead eyebrow="AI AGENTS" title="Agents" subtitle="Agent هر نقش، فقط در محدوده Capabilityهای همان کاربر عمل می‌کند."/>{error&&<p className="error-text">{error}</p>}<div className="agent-grid">{items.map(a=><Card key={a.id}><div className="agent-orb">✦</div><h3>{a.name}</h3><p>{a.description||"Role-aware enterprise agent"}</p><div><Pill>{a.tools} tools</Pill><Pill tone="success">Policy bound</Pill></div></Card>)}{!items.length&&!error&&<Card><Empty text="Agentی ثبت نشده است."/></Card>}</div></>}
