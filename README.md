...
- **`10_setup_buzz.sh`** (مورد ۵۹): clone + build محلی
  ...
  اندپوینت (`/api/bootstrap`, `/api/me/capabilities`, `/api/chat`) عبور کنه.
...
stdio که دقیقاً همون سه ابزار `gateway_bootstrap` / `gateway_capabilities` /
`gateway_chat` را (هرکدام یک forward ساده به endpoint نام‌گذاری‌شده) در
...
5. قبل از فراتر رفتن از یک کانال pilot، هر سه فراخوانی پل
   (`gateway_bootstrap`/`gateway_capabilities`/`gateway_chat`) رو در Audit Log
